...
for CUR in src/config/*.cursor; do
	...
	if [ "$DIR1X/$BASENAME.png" -ot $RAWSVG ] ; then
		inkscape -i $BASENAME -d 90  -o "$DIR1X/$BASENAME.png" $RAWSVG > /dev/null 
	fi

	if [ "$DIR1_5X/$BASENAME.png" -ot $RAWSVG ] ; then
		inkscape -i $BASENAME -d 135 -o "$DIR1_5X/$BASENAME.png" $RAWSVG > /dev/null
	fi

	if [ "$DIR2X/$BASENAME.png" -ot $RAWSVG ] ; then
		inkscape -i $BASENAME -d 180 -o "$DIR2X/$BASENAME.png" $RAWSVG > /dev/null
	fi
done
...
do
	...
	if [ "$DIR1X/progress-$i.png" -ot $RAWSVG ] ; then
		inkscape -i progress-$i -d 90  -o "$DIR1X/progress-$i.png" $RAWSVG > /dev/null
	fi

    if [ "$DIR1_5X/progress-$i.png" -ot $RAWSVG ] ; then
        inkscape -i progress-$i -d 135 -o "$DIR1_5X/progress-$i.png" $RAWSVG > /dev/null
	fi

	if [ "$DIR2X/progress-$i.png" -ot $RAWSVG ] ; then
		inkscape -i progress-$i -d 180 -o "$DIR2X/progress-$i.png" $RAWSVG > /dev/null
	fi

	if [ "$DIR1X/wait-$i.png" -ot $RAWSVG ] ; then
		inkscape -i wait-$i -d 90  -o "$DIR1X/wait-$i.png" $RAWSVG > /dev/null
	fi

	if [ "$DIR1_5X/wait-$i.png" -ot $RAWSVG ] ; then
		inkscape -i wait-$i -d 135  -o "$DIR1_5X/wait-$i.png" $RAWSVG > /dev/null
	fi

	if [ "$DIR2X/wait-$i.png" -ot $RAWSVG ] ; then
		inkscape -i wait-$i -d 180 -o "$DIR2X/wait-$i.png" $RAWSVG > /dev/null
	fi
done
